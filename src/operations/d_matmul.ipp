...
void matmul (VARR dest, std::vector<VARR> srcs, ARGS)
{
	...
	std::vector<size_t> alist = srcshape0.as_list();
	std::vector<size_t> blist = srcs.back().second.as_list();
	T* d = dest.first;
	...
	size_t beyond2d = srcshape0.n_elems() / (dim_z * dim_y);
	...
	if (dim_pad> STRASSEN_THRESHOLD)
	{
		...
		{
			const T* rawa = sa + i * (dim_z * dim_y);
			const T* rawb = sb + i * (dim_x * dim_z);
			T* rawc = d + i * (dim_x * dim_y);
