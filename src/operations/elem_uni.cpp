...
varptr identity (varptr x)
{
	...
	varptr out = linear::get(std::vector<inode*>{x}, unary_shaper,
	new actor_func(
...
{
	if (nullptr == x.get()) return nullptr;
	std::string opname = "const_con";
	if (inode* parent = unary_parent_search(x.get(), opname))
	{
		return parent;
	}
	varptr out = const_con::get(x);
	out->extract_metadata(x.get());
	return out;
}
...
varptr operator + (const varptr a)
{
	...
	varptr out = linear::get(std::vector<inode*>{a}, unary_shaper,
	new actor_func(
	CONN_ACTOR([](out_wrapper<void>& dest, std::vector<in_wrapper<void> >& srcs, 
		TENS_TYPE type) -> itens_actor*
	{
...
{
	if (nullptr == base.get()) return nullptr;
	if (xponent == 0)
	{
		return constant::get((double) 1); // todo: shape matters here
	}
	else if (xponent == 1)
	{
		return base;
	}
	if (constant* bconst = dynamic_cast<constant*>(base.get()))
	{
		std::vector<double> bcvec = expose<double>(bconst);
		for (double& bcv : bcvec)
		{
			bcv = std::pow(bcv, xponent);
		}
		return constant::get(bcvec, bconst->get_shape());
	}
	std::string opname = nnutils::formatter() << "pow_" << xponent;
	if (inode* parent = unary_parent_search(base.get(), opname))
	{
		return parent;
	}
	varptr out = linear::get(std::vector<inode*>{base}, unary_shaper,
	new actor_func(
	CONN_ACTOR([xponent](out_wrapper<void>& dest, std::vector<in_wrapper<void> >& srcs,
		TENS_TYPE type) -> itens_actor*
	{
		switch (type)
		{
			case DOUBLE:
				return new tens_pow_uni<double>(dest, srcs, xponent);
			case INT:
				return new tens_pow_uni<signed>(dest, srcs, xponent);
			default:
			break;
		}
		return nullptr;
	})),
	[xponent](std::vector<std::pair<inode*,inode*>> args)
	{
		// pow'(f(x), c) = f'(x) * (c*f(x)^(c-1))
		varptr a = args.front().first;
		varptr grad = args.front().second;
		return xponent * grad * pow(a, xponent - 1);
	}, opname);
	out->extract_metadata(base.get());
	return out;
}

varptr pow (double base, const varptr xponent)
{
	if (nullptr == xponent.get()) return nullptr;
	if (base == 0)
	{
		return constant::get((double) 0); // todo: shape matters here
	}
	else if (base == 1)
	{
		return constant::get((double) 1);
	}
	if (constant* xconst = dynamic_cast<constant*>(xponent.get()))
	{
		std::vector<double> xcvec = expose<double>(xconst);
		for (double& xcv : xcvec)
		{
			xcv = std::pow(base, xcv);
		}
		return constant::get(xcvec, xconst->get_shape());
	}
	std::string opname = nnutils::formatter() << base << "_pow";
	if (inode* parent = unary_parent_search(xponent.get(), opname))
	{
		return parent;
	}
	varptr out = linear::get(std::vector<inode*>{xponent}, unary_shaper,
	new actor_func(
	CONN_ACTOR([base](out_wrapper<void>& dest, std::vector<in_wrapper<void> >& srcs,
		TENS_TYPE type) -> itens_actor*
	{
		switch (type)
		{
			case DOUBLE:
				return new tens_pow_uni<double>(base, dest, srcs);
			case INT:
				return new tens_pow_uni<signed>(base, dest, srcs);
			default:
			break;
		}
		return nullptr;
	})),
	[base](std::vector<std::pair<inode*,inode*>> args)
	{
		// pow'(c, f(x)) = pow(c, f(x)) * ln(c)
		varptr me = pow(base, args.front().first);
		return me * std::log(base);
	}, opname);
...
varptr clip (const varptr a, double min, double max)
{
	...
	varptr out = nlinear::get(std::vector<inode*>{a}, unary_shaper,
	new actor_func(
...
varptr clip_norm (const varptr a, double cap)
{
	...
	varptr norm = reduce_l2norm(a);
	varptr clipped = a * cap / norm;
	std::string opname = nnutils::formatter() << "clip_l2norm_" << cap;
	if (inode* parent = unary_parent_search(a.get(), opname))
	{
		return parent;
	}
	varptr out = nlinear::get(std::vector<inode*>{a, clipped, norm}, unary_shaper,
	new actor_func(
	CONN_ACTOR([cap](out_wrapper<void>& dest, std::vector<in_wrapper<void> >& srcs,
		TENS_TYPE type) -> itens_actor*
	{
		assert(2 == srcs.size());
		const void* l2norm = srcs[2].first;
		switch (type)
		{
			case DOUBLE:
				return new tens_clip_norm<double>(dest, {srcs[0], srcs[1]}, l2norm, cap);
			case INT:
				return new tens_clip_norm<signed>(dest, {srcs[0], srcs[1]}, l2norm, cap);
			default:
			break;
		}
		return nullptr;
	...
	{
		varptr a = args.front().first; 
		// tensorshape as = a->get_shape();
		// return varptr(constant::get(std::vector<double>(1, as.n_elems()), as)) * cap / reduce_l2norm(a) + cap * reduce_l2norm(a);
		varptr grad = args.front().second;
...
{
	if (nullptr == a.get()) return nullptr;
	if (inode* parent = unary_parent_search(a.get(), name))
	{
		return parent;
	}
	varptr out = linear::get(std::vector<inode*>{a}, unary_shaper,
		...
		TENS_TYPE type) -> itens_actor*
	{
		...
		{
			case DOUBLE:
				return new tens_conditional_uni<double>(dest, srcs, compare);
			case INT:
				return new tens_conditional_uni<signed>(dest, srcs, compare);
			default:
			break;
		}
		return nullptr;
...
varptr conditional (double a, const varptr b, COMPARE<double> compare, std::string name)
{
	...
	return conditional(b, [a, compare](double b)
	{
		return compare(a, b);
	}, opname);
}
...
varptr conditional (const varptr a, double b, COMPARE<double> compare, std::string name)
{
	...
	return conditional(a, [b, compare](double a)
	{
		return compare(a, b);
	}, opname);
}
...
varptr binomial_sample (signed n, const varptr p)
{
	...
	return linear::get(std::vector<inode*>{p}, unary_shaper,
	new actor_func(
...
varptr binomial_sample (const varptr n, double p)
{
	...
	return linear::get(std::vector<inode*>{n}, unary_shaper,
	new actor_func(
...
varptr operator - (double a, const varptr b)
{
	...
	varptr out = linear::get(std::vector<inode*>{b}, unary_shaper,
	new actor_func(
