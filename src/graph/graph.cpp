...
void graph::serialize (tenncor::GraphPb& proto_dest) const
{
	proto_dest.set_gid(gid_);
	proto_dest.clear_node_map();
	proto_dest.clear_create_order();
	// set GraphPb node_map (1)
	google::protobuf::Map<std::string,tenncor::NodePb>& nmap = *(proto_dest.mutable_node_map());
...
static inline variable* make_variable (tenncor::VariablePb var_src, std::string label)
{
	const tenncor::SourcePb& source_src = var_src.source();
...
static inline placeholder* make_placeholder (tenncor::PlacePb& place_src, std::string label)
{
	...
	const tenncor::GraphPb& proto_src)
{
	...
	adjmap_.clear();
	order_.clear();
	const google::protobuf::Map<std::string,tenncor::NodePb>& nmap = proto_src.node_map();
	...
	{
		std::string uid = *it;
		const tenncor::NodePb& node_src = nmap.at(uid);
...
bool graph::save_data (tenncor::DataRepoPb& proto_dest) const
{
...
void graph::load_data (const tenncor::DataRepoPb& proto_src)
{
