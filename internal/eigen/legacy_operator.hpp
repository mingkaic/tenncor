...
{
	return std::make_shared<LegacyTensOp<T,SRC,ARGS>>(
		dims, args, make_base);
}
...
inline EigenptrT make_eigenmatrix (DimensionsT dims, ARGS args,
	std::function<SRC(ARGS&)> make_base)
{
	return std::make_shared<LegacyMatOp<T,SRC,ARGS>>(
		dims, args, make_base);
}

template <typename T>
T* get_data (const teq::iTensor& src)
{
	return (T*) src.device().odata().get();
}
...
outdims, make_tensmap(get_data<T>(in), in.shape()), [&ARR, &outdims](TensMapT<T>& in) {\
		...
		return make_eigentensor<T,Eigen::TensorReshapingOp<
			...
			const Eigen::TensorConversionOp<T,
				const Eigen::TensorTupleReducerOp<
					...
					const TensMapT<T>>>>,
			TensMapT<T>>(
			outdims, make_tensmap(get_data<T>(in), in.shape()),
...
EigenptrT extend (teq::Shape outshape, const teq::iTensor& in, const marsh::iAttributed& attrib)
{
	...
	return make_eigentensor<T,Eigen::TensorBroadcastingOp<
		...
		shape_convert(outshape), make_tensmap(get_data<T>(in), inshape),
		[coord](TensMapT<T>& in)
		{
...
EigenptrT permute (teq::Shape outshape, const teq::iTensor& in, const marsh::iAttributed& attrib)
{
	...
	if (is_2d(outshape) && reorder[0] == 1 && reorder[1] == 0)
	{
		...
		return make_eigenmatrix<T,Eigen::Transpose<MatMapT<T>>,
			MatMapT<T>>(
			shape_convert(outshape), make_matmap(get_data<T>(in), in.shape()),
			[](MatMapT<T>& in)
			{
				return in.transpose();
			});
	...
	return make_eigentensor<T,Eigen::TensorShufflingOp<
		...
		shape_convert(outshape), make_tensmap(get_data<T>(in), in.shape()),
...
EigenptrT slice (teq::Shape outshape, const teq::TensptrT& in, const marsh::iAttributed& attrib)
{
	...
	return make_eigentensor<T,Eigen::TensorReshapingOp<
		const DimensionsT, Eigen::TensorSlicingOp<
			...
			TensMapT<T>>>,
		TensMapT<T>>(
		outdims, make_tensmap(get_data<T>(*in), shape),
...
EigenptrT scatter (teq::Shape outshape, const teq::iTensor& in, const marsh::iAttributed& attrib)
{
	...
	return std::make_shared<TensAccum<T,TensMapT<T>>>(
		0, shape_convert(outshape), make_tensmap(get_data<T>(in), in.shape()),
...
EigenptrT concat (teq::Shape outshape, const teq::TensptrsT& group, const marsh::iAttributed& attrib)
{
	...
	if (group.size() == 2)
	{
		...
		return make_eigentensor<T,
			...
			shape_convert(outshape), {
				make_tensmap(get_data<T>(*left), left->shape()),
				make_tensmap(get_data<T>(*right), right->shape())},
	...
	std::transform(group.begin(), group.end(), std::back_inserter(args),
		[](teq::TensptrT arg)
		{
			return make_tensmap(get_data<T>(*arg), arg->shape());
		});
...
EigenptrT abs (teq::Shape outshape, const teq::iTensor& in)
{
	if (is_2d(outshape))
	{
		...
		return make_eigenmatrix<T,Eigen::CwiseUnaryOp<
			...
			make_matmap(get_data<T>(in), in.shape()),
			[](MatMapT<T>& in)
			{
				return in.cwiseAbs();
			});
	...
	return make_eigentensor<T,Eigen::TensorCwiseUnaryOp<
		...
		make_tensmap(get_data<T>(in), in.shape()),
		[](TensMapT<T>& in)
		{
			return in.abs();
		});
...
EigenptrT pow (teq::Shape outshape, const teq::iTensor& a, const teq::iTensor& b)
{
	if (is_2d(outshape))
	{
		...
		return make_eigenmatrix<T,Eigen::CwiseBinaryOp<
			...
			std::vector<MatMapT<T>>>(shape_convert(outshape), {
				make_matmap(get_data<T>(a), a.shape()),
				make_matmap(get_data<T>(b), b.shape())},
	...
	return make_eigentensor<T,Eigen::TensorCwiseBinaryOp<
		...
		std::vector<TensMapT<T>>>(shape_convert(outshape), {
			make_tensmap(get_data<T>(a), a.shape()),
			make_tensmap(get_data<T>(b), b.shape())},
		[](std::vector<TensMapT<T>>& args)
		{
			return args[0].binaryExpr(args[1],
				std::function<T(const T&,const T&)>(
	...
	const teq::iTensor& then, const teq::iTensor& otherwise)
{
	if (is_2d(outshape))
	{
		...
		return make_eigenmatrix<T,Eigen::Select<MatMapT<T>,
			...
			shape_convert(outshape), {
			make_matmap(get_data<T>(condition), condition.shape()),
			make_matmap(get_data<T>(then), then.shape()),
			make_matmap(get_data<T>(otherwise), otherwise.shape())},
			[](std::vector<MatMapT<T>>& args)
			{
	...
	return make_eigentensor<T,
		Eigen::TensorSelectOp<const TensMapT<T>,
			const TensMapT<T>,const TensMapT<T>>,
		std::vector<TensMapT<T>>>(shape_convert(outshape), {
			make_tensmap(get_data<T>(condition), condition.shape()),
			make_tensmap(get_data<T>(then), then.shape()),
			make_tensmap(get_data<T>(otherwise), otherwise.shape())},
		[](std::vector<TensMapT<T>>& args)
		{
			return args[0].select(args[1], args[2]);
		});
...
EigenptrT matmul (teq::Shape outshape, const teq::iTensor& a, const teq::iTensor& b)
{
	...
	if (!is_2d(ashape) && !is_2d(bshape))
	{
		...
		size_t asize = as.n_elems();
		size_t bsize = bs.n_elems();
		auto adata = get_data<T>(a);
		auto bdata = get_data<T>(b);
		std::vector<MatMapT<T>> args;
		args.reserve(nbatches * 2);
		for (size_t i = 0; i < nbatches; ++i)
		{
			args.push_back(make_matmap(adata + i * asize, as));
			args.push_back(make_matmap(bdata + i * bsize, bs));
		}
		return std::make_shared<TensAccum<T,std::vector<MatMapT<T>>>>(
		0, shape_convert(outshape), args,
		[os,nbatches](TensorT<T>& out, const std::vector<MatMapT<T>>& args)
		{
			auto osize = os.n_elems();
			auto odata = out.data();
			for (size_t i = 0; i < nbatches; ++i)
			{
				make_matmap<T>(odata + i * osize, os) = args[i * 2] * args[i * 2 + 1];
			}
		});
	}
	...
	const teq::iTensor& kernel, const marsh::iAttributed& attrib)
{
	...
	return make_eigentensor<T,Eigen::TensorConvolutionOp<
		...
		const TensMapT<T>,const TensMapT<T>>,
		std::vector<TensMapT<T>>>(shape_convert(outshape), {
			make_tensmap(get_data<T>(input), input.shape()),
			make_tensmap(get_data<T>(kernel), kernel.shape())},
		[&](std::vector<TensMapT<T>>& args)
		{
...
using EigenConvertT = Eigen::TensorConversionOp<OUTTYPE,const TensMapT<INTYPE>>;
...
get_data<INTYPE>(*input), input->shape()),\
[&](TensMapT<INTYPE>& arg) -> EigenConvertT<T,INTYPE> {\
