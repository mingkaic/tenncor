...
void set_logger (logs::iLogger* logger, CfgMapptrT ctx = context());

logs::iLogger& get_logger (const CfgMapptrT& ctx = context());
...
/// Return log level
std::string get_log_level (const CfgMapptrT& ctx = context());
