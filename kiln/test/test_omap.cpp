...
TEST_F(OMAP, PutGet_A000)
{
	...
	kiln::OrderedMap<std::string, size_t> omap;

	kiln::optional<size_t> nf = omap.get(key);
	EXPECT_FALSE((bool) nf);
	EXPECT_TRUE(omap.put(key, value));

	kiln::optional<size_t> gotv = omap.get(key);
...
TEST_F(OMAP, PutReplace_A002)
{
	...
	kiln::optional<size_t> gotv2 = omap.get(key);
