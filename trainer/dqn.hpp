...
struct DQNTrainer final
{
	DQNTrainer (eteq::ETensor<T>& model,
		...
		sess_(&sess), source_model_(model), target_model_(eteq::deep_clone(model)),
	...
	void train (void)
	{
		...
		if (0 == (ntrain_called_ % train_interval_))
		{
			...
			src_input_->assign(states.data(), src_input_->shape(), *sess_);
			src_outmask_->assign(action_mask.data(), src_outmask_->shape(), *sess_);
			nxt_input_->assign(new_states.data(), nxt_input_->shape(), *sess_);
			reward_input_->assign(rewards.data(), reward_input_->shape(), *sess_);
