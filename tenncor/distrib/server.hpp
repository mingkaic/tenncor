...
struct DistrServer final
{
	...
	// non-blocking
	DistrServer (iDistrEvaluator* eval, PartDerF derive,
		...
		derive_(derive), alias_(alias)
	{
		...
		std::thread rpc_job(&DistrServer::handle_rpcs, this, eval);
		rpc_job.detach();
	}
...
private:
	...
	void handle_rpcs (iDistrEvaluator* eval)
	{
		// FindNodes
		...
		{
			return this->find_nodes(eval, req, res);
		}, cq_.get());
		...
		[this, eval](
			...
			DeriveResponse& res)
		{
			return this->derive(eval, req, res);
		}, cq_.get());
		...
		[this, eval](DataStatesT& states, const GetDataRequest& req) -> grpc::Status
		{
			...
			for (const std::string& uuid : uuids)
			{
				...
				auto tens = eval->lookup_node(err, uuid, false).get();
	...
	grpc::Status find_nodes (iDistrEvaluator* eval,
		const FindNodesRequest& req,
		FindNodesResponse& res)
	{
		auto& uuids = req.uuids();
		...
		{
			error::ErrptrT err = nullptr;
			auto node = eval->lookup_node(err, uuid, false);
			...
			{
				out->set_instance(eval->get_id());
			}
		}
		return grpc::Status::OK;
	}

	grpc::Status derive (iDistrEvaluator* eval,
		const DeriveRequest& req,
		DeriveResponse& res)
	{
		...
		for (auto& reqpairs : rgrads)
		{
			...
			auto local = eval->lookup_node(err, local_id, false);
			ERR_CHECK(err, grpc::NOT_FOUND, alias_.c_str());
			auto grad = eval->lookup_node(err, grad_id);
			ERR_CHECK(err, grpc::NOT_FOUND, alias_.c_str());
			grads[local.get()].push_back(grad);
			parents.emplace(local);
			auto rec = reachable_refs(teq::TensT{local.get()});
			auto owners = teq::track_owners({local});
			for (auto ref : rec)
			{
				reachables.emplace(std::dynamic_pointer_cast<
					distr::iDistRef>(owners[ref].lock()));
			}
		}
		for (auto targid : targids)
		{
			auto target = eval->lookup_node(err, targid);
			ERR_CHECK(err, grpc::NOT_FOUND, alias_.c_str());
		...
		for (auto targ : tgrads)
		{
			...
			std::string gid = *eval->lookup_id(grad);
