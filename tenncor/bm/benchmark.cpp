...
static void BM_MatmulComplex(benchmark::State& state)
{
	...
	for (auto _ : state)
	{
		...
		state.ResumeTiming();
		a->assign(data.data(), a->shape(), eigen::global_context());
		b->assign(data2.data(), b->shape(), eigen::global_context());
		c->assign(data3.data(), c->shape(), eigen::global_context());
		da.calc();
		db.calc();
		dc.calc();
	}
...
static void BM_SigmoidMLP(benchmark::State& state)
{
	...
	for (auto _ : state)
	{
		...
		state.ResumeTiming();
		in->assign(in_data.data(), in->shape(), eigen::global_context());
		out->assign(out_data.data(), out->shape(), eigen::global_context());
		weight0->assign(w0_data.data(), weight0->shape(), eigen::global_context());
		bias0->assign(b0_data.data(), bias0->shape(), eigen::global_context());
		weight1->assign(w1_data.data(), weight1->shape(), eigen::global_context());
		bias1->assign(b1_data.data(), bias1->shape(), eigen::global_context());
		dw0.calc();
		db0.calc();
		dw1.calc();
		db1.calc();
