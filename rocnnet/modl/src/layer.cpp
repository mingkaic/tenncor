...
std::pair<std::string,LayerId> unpack_label (std::string label)
{
	...
	return {parts[0], LayerId(parts[1], parts[2], std::stoul(parts[3]))};
}
			...
			if (estd::get(subid, layer_info, val_))
			{
				...
				while (false == subid.type_.empty())
				{
					...
					else
					{
						...
						std::string subprefix = subid.label_ + llabel_sep;
						auto it = std::find_if(sublabels.begin(), sublabels.end(),
						[&subprefix](std::string label)
						{
							return subprefix.end() == std::mismatch(label.begin(), label.end(),
								subprefix.begin()).first;
						});
						if (sublabels.end() == it)
						{
							logs::fatalf("failed to find sublabel %s in sublayer %s",
								subid.label_.c_str(), subid.type_.c_str());
						}
						subid = unpack_label(*it).second;
					}
				}
		...
		else
		{
			...
			if (id.index_ >= n)
			{
				grouped_layers.insert(grouped_layers.end(), id.index_ - n + 1,
					std::pair<LBuilderptrT,SublayersT>());
			}
	...
	LayerRegistry& registry)
{
	ade::OwnerMapT owners = ade::track_owners(
		ade::TensT(graph.roots_.begin(), graph.roots_.end()));

	LayerDeserializer layd(ltype, label);
	// get all layer labelled nodes in graph
	for (ade::TensptrT tens : graph.roots_)
	{
		tens->accept(layd);
	}
