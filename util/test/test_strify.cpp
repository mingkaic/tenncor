...
{
	std::stringstream ss;
	to_stream(ss, "abcdefghijkl");
	EXPECT_STREQ("abcdefghijkl", ss.str().c_str());
	ss.str("");

	to_stream(ss, "abcd\\efgh\\ijkl\\");
...
{
	std::stringstream ss;
	to_stream(ss, "hey what's the answer to number", 15,
