...
def grpc_proto_library(name, src, has_service = True):
    ...
    command = "$(location protocol_compiler) --cpp_out=$(GENDIR)/"
    ...
    native.genrule(
        ...
        cmd = command,
        outs = outputs,
        tools = ['protocol_compiler'],
    )

    if has_service:
            grpc_command = "$(location protocol_compiler) --grpc_out=$(GENDIR)/"
            grpc_command += " --plugin=protoc-gen-grpc=$(location grpc_cpp_plugin)"
