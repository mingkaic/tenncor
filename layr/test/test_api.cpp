...
TEST(CONNECT, TanhRNNTraining)
{
	...
	for (auto root : roots)
	{
		momentums.push_back(eteq::make_variable_like<double>(0, root));
		mvavg_sqrs.push_back(eteq::make_variable_like<double>(0, root));
	}
	...
	// group 2
	{
		...
		for (size_t i = 0; i < nroots; ++i)
		{
			group1_left[i]->assign(*group2_right[i]);
		}
	...
	// group 3
	{
		...
		{
			group1_left[i]->assign(*group3_right[i]);
		}
