...
{
	std::string label = get_string(get_int(1, "label.size", {14, 29})[0], "label1");
	nnet::tensorshape shape = random_def_shape(this);
	std::shared_ptr<mock_data_src> src = std::make_shared<mock_data_src>(this);
	nnet::variable res(shape, src, label);

	std::unordered_set<nnet::inode*> leafset = res.get_leaves();
...
{
	std::string label = get_string(get_int(1, "label1.size", {14, 29})[0], "label1");
	nnet::tensorshape shape = random_def_shape(this);
	double c = get_double(1, "c")[0];
