...
TEST_F(TYPE, TypeConvert_B002)
{
	...
	EXPECT_THROW(nnet::type_convert<double>(&datad[0], datad.size(), BAD_T), nnutils::unsupported_type_error);
	EXPECT_THROW(nnet::type_convert<double>(&datai[0], datai.size(), BAD_T), nnutils::unsupported_type_error);

	std::vector<double> dvec = nnet::type_convert<double>(&datad[0], datad.size(), DOUBLE);
	std::vector<float> fvec = nnet::type_convert<float>(&datad[0], datad.size(), DOUBLE);
	std::vector<int8_t> i8vec = nnet::type_convert<int8_t>(&datad[0], datad.size(), DOUBLE);
	std::vector<uint8_t> u8vec = nnet::type_convert<uint8_t>(&datad[0], datad.size(), DOUBLE);
	std::vector<int16_t> i16vec = nnet::type_convert<int16_t>(&datad[0], datad.size(), DOUBLE);
	std::vector<uint16_t> u16vec = nnet::type_convert<uint16_t>(&datad[0], datad.size(), DOUBLE);
	std::vector<int32_t> i32vec = nnet::type_convert<int32_t>(&datad[0], datad.size(), DOUBLE);
	std::vector<uint32_t> u32vec = nnet::type_convert<uint32_t>(&datad[0], datad.size(), DOUBLE);
	std::vector<int64_t> i64vec = nnet::type_convert<int64_t>(&datad[0], datad.size(), DOUBLE);
	std::vector<uint64_t> u64vec = nnet::type_convert<uint64_t>(&datad[0], datad.size(), DOUBLE);

	std::vector<double> dvec2 = nnet::type_convert<double>(&datai[0], datai.size(), UINT64);
	std::vector<float> fvec2 = nnet::type_convert<float>(&datai[0], datai.size(), UINT64);
	std::vector<int8_t> i8vec2 = nnet::type_convert<int8_t>(&datai[0], datai.size(), UINT64);
	std::vector<uint8_t> u8vec2 = nnet::type_convert<uint8_t>(&datai[0], datai.size(), UINT64);
	std::vector<int16_t> i16vec2 = nnet::type_convert<int16_t>(&datai[0], datai.size(), UINT64);
	std::vector<uint16_t> u16vec2 = nnet::type_convert<uint16_t>(&datai[0], datai.size(), UINT64);
	std::vector<int32_t> i32vec2 = nnet::type_convert<int32_t>(&datai[0], datai.size(), UINT64);
	std::vector<uint32_t> u32vec2 = nnet::type_convert<uint32_t>(&datai[0], datai.size(), UINT64);
	std::vector<int64_t> i64vec2 = nnet::type_convert<int64_t>(&datai[0], datai.size(), UINT64);
	std::vector<uint64_t> u64vec2 = nnet::type_convert<uint64_t>(&datai[0], datai.size(), UINT64);
