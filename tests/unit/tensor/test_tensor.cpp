...
{
	nnet::tshape exshape(random_shape(this));
...
{
	nnet::tshape pshape(random_undef_shape(this));
	nnet::tshape cshape(random_def_shape(this));
...
// cover tensor: read_from(const idata_src& src, const tshape shape), get_type, has_data
TEST_F(TENSOR, ReadFromShape_C001)
{
	...
	nnet::tshape pshape(plist);
	nnet::tshape good_shape(goodlist);

	nnet::tshape cshape(clist);
	nnet::tshape bad_shape(badlist);
...
TEST_F(TENSOR, Copy_C002)
{
	...
	nnet::tshape expectc = undefcpy.get_shape();
	ASSERT_TRUE(tshape_equal(cshape, expectc)) <<
		testutils::sprintf("expecting shape %p, got %p", &cshape, &expectc);
	nnet::tshape expectp = incomcpy.get_shape();
	ASSERT_TRUE(tshape_equal(pshape, expectp)) <<
		testutils::sprintf("expecting shape %p, got %p", &pshape, &expectp);
	nnet::tshape expectc2 = compcpy.get_shape();
	...
	nnet::tshape expectc3 = undefassign.get_shape();
	ASSERT_TRUE(tshape_equal(cshape, expectc3)) <<
		testutils::sprintf("expecting shape %p, got %p", &cshape, &expectc3);
	nnet::tshape expectp2 = incomassign.get_shape();
	ASSERT_TRUE(tshape_equal(pshape, expectp2)) <<
		testutils::sprintf("expecting shape %p, got %p", &pshape, &expectp2);
	nnet::tshape expectc4 = compassign.get_shape();
	ASSERT_TRUE(tshape_equal(cshape, expectc4)) <<
...
TEST_F(TENSOR, Move_C002)
{
	...
	nnet::tshape s0 = undef.get_shape();
	nnet::tshape s1 = incom.get_shape();
	nnet::tshape s2 = comp.get_shape();
	...
	nnet::tshape expectc = undefmv.get_shape();
	ASSERT_TRUE(tshape_equal(cshape, expectc)) <<
		testutils::sprintf("expecting shape %p, got %p", &cshape, &expectc);
	nnet::tshape expectp = incommv.get_shape();
	ASSERT_TRUE(tshape_equal(pshape, expectp)) <<
		testutils::sprintf("expecting shape %p, got %p", &pshape, &expectp);
	nnet::tshape expectc2 = compmv.get_shape();
	EXPECT_SHAPEQ(cshape,  expectc2);
	...
	nnet::tshape s3 = undefmv.get_shape();
	nnet::tshape s4 = incommv.get_shape();
	nnet::tshape s5 = compmv.get_shape();
...
TEST_F(TENSOR, ShapeAccessor_C003)
{
	...
	nnet::tshape pshape = pds;
	nnet::tshape cshape = cds;
	nnet::tensor ten(pshape);

	nnet::tshape expshape = ten.get_shape();
	...
	nnet::tshape excshape = ten.get_shape();
...
TEST_F(TENSOR, IsSameSize_C004)
{
	...
	nnet::tshape cshape = clist;
	nnet::tshape pshape = plist;
	nnet::tshape badpshape = badpartial;
	...
	nnet::tshape p1 = padd(clist, npads[0], npads[1]); // same
	nnet::tshape p2 = padd(clist, npads[2], npads[3]); // same
	clist.push_back(2);
	nnet::tshape p3 = padd(clist, npads[2], npads[3]); // not same
...
TEST_F(TENSOR, GuessShape_C007)
{
	...
	optional<nnet::tshape> cres = comp.guess_shape(exactdata);
	...
	nnet::tshape pmod = plist;
	nnet::tshape pshape2 = plist2;
	// allowed are partially defined
	optional<nnet::tshape> pres = pcom.guess_shape(exactdata2);
	optional<nnet::tshape> pres2 = pcom.guess_shape(moddata);
	...
	optional<nnet::tshape> ures = undef.guess_shape(exactdata);
	optional<nnet::tshape> ures2 = undef.guess_shape(exactdata2);
	optional<nnet::tshape> ures3 = undef.guess_shape(lowerdata);
	optional<nnet::tshape> ures4 = undef.guess_shape(lowerdata2);
	optional<nnet::tshape> ures5 = undef.guess_shape(upperdata);
	optional<nnet::tshape> ures6 = undef.guess_shape(upperdata2);
	optional<nnet::tshape> ures7 = undef.guess_shape(moddata);
	...
	EXPECT_TRUE(tshape_equal(*ures, std::vector<size_t>({exactdata}))) <<
		testutils::sprintf("expected shape %d, got %p", exactdata, &*ures);
	EXPECT_TRUE(tshape_equal(*ures2, std::vector<size_t>({exactdata2}))) <<
		testutils::sprintf("expected shape %d, got %p", exactdata2, &*ures2);
	EXPECT_TRUE(tshape_equal(*ures3, std::vector<size_t>({lowerdata}))) <<
		testutils::sprintf("expected shape %d, got %p", lowerdata, &*ures3);
	EXPECT_TRUE(tshape_equal(*ures4, std::vector<size_t>({lowerdata2}))) <<
		testutils::sprintf("expected shape %d, got %p", lowerdata2, &*ures4);
	EXPECT_TRUE(tshape_equal(*ures5, std::vector<size_t>({upperdata}))) <<
		testutils::sprintf("expected shape %d, got %p", upperdata, &*ures5);
	EXPECT_TRUE(tshape_equal(*ures6, std::vector<size_t>({upperdata2}))) <<
		testutils::sprintf("expected shape %d, got %p", upperdata2, &*ures6);
	EXPECT_TRUE(tshape_equal(*ures7, std::vector<size_t>({moddata}))) <<
...
{
	mock_data_src src(this);
	nnet::tshape shape = std::vector<size_t>{(size_t) (16 / nnet::type_size(src.type_))};
...
TEST_F(TENSOR, Clear_C010)
{
	...
	nnet::tshape tshape = ten.get_shape();
...
TEST_F(TENSOR, SetShape_C011)
{
	...
	comp.clear();
	pcom.clear();
	nnet::tshape res_ushape = undef.get_shape();
	nnet::tshape res_cshape = comp.get_shape();
	nnet::tshape res_pshape = pcom.get_shape();
	...
	nnet::tshape res_ushape2 = undef2.get_shape();
	nnet::tshape res_cshape2 = comp2.get_shape();
	nnet::tshape res_pshape2 = pcom2.get_shape();
...
TEST_F(TENSOR, Proto_C012)
{
	...
	nnet::tshape cshape = random_def_shape(this);
	nnet::tshape cshape2 = random_def_shape(this);
	std::vector<size_t> clist = random_def_shape(this);
	nnet::tshape cshape3(clist);
	nnet::tshape pshape3 = make_partial(this, clist);
	nnet::tshape shape = random_shape(this);
	nnet::tensor comp(cshape);
	...
	nnet::tshape goten = ten.get_shape();
	nnet::tshape gotc = comp2.get_shape();
	...
	nnet::tshape c3allow(std::vector<size_t>(
		proto.allowed_shape().begin(),
		proto.allowed_shape().end()));
	nnet::tshape c3alloc(std::vector<size_t>(
