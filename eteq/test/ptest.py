...
class EADTest(unittest.TestCase):
    ...
    def _common_unary(self, shape, api, real, derive):
        ...
        ex = eteq.derive(out, eteq.link(var))
        zero = eteq.derive(out, eteq.link(var2))
    ...
    def _common_binary(self, shape, api, real, derive):
        ...
        out = api(eteq.link(var), eteq.link(var2))
        both = api(eteq.link(var), eteq.link(var))
        clhs = api(eteq.link(var), cst)
        crhs = api(cst2, eteq.link(var2))
        ...
        zero = eteq.derive(out, eteq.link(var3))
        ex = eteq.derive(out, eteq.link(var))
        ex2 = eteq.derive(out, eteq.link(var2))
        ex3 = eteq.derive(both, eteq.link(var))
        ex4 = eteq.derive(clhs, eteq.link(var))
        ex5 = eteq.derive(crhs, eteq.link(var2))
    ...
    def _common_reduce_1d(self, dim_reduce, tf_reduce):
        ...
        out = dim_reduce(eteq.link(var), 1)
    ...
    def _common_reduce(self, all_reduce, dim_reduce, tf_reduce):
        ...
        for shape in shapes:
            ...
            out = all_reduce(eteq.link(var))
            out2 = dim_reduce(eteq.link(var), offset=1)
            ...
            var2 = eteq.variable(data, 'var2')
            ex = eteq.derive(out, eteq.link(var))
            ex2 = eteq.derive(out2, eteq.link(var))
            zero = eteq.derive(out, eteq.link(var2))
    ...
    def _common_argreduce(self, dim_reduce, tf_reduce):
        ...
        for shape in shapes:
            ...
            out = tc.permute(dim_reduce(eteq.link(var),
                return_dim=1), [0, 2, 1])
            tf_out = tf_reduce(tf_var, 1)
    ...
    def test_variable(self):
        ...
        for shape in shapes:
            ...
            sess = eteq.Session()
            sess.track([eteq.link(var)])
            sess.update()
            fout = eteq.link(var).get()
            ...
            var2 = eteq.variable(data, 'var2')
            one = eteq.derive(eteq.link(var), eteq.link(var))
            zero = eteq.derive(eteq.link(var), eteq.link(var2))
            sess.track([one, zero])
            sess.update()
    ...
    def test_extend(self):
        ...
        out = tc.extend(eteq.link(var), 1, [3])
        sess = eteq.Session()
        sess.track([out])
        sess.update()
    ...
    def test_rl2norm(self):
        ...
        for shape in shapes:
            ...
            out = tc.reduce_l2norm(eteq.link(var))
    ...
    def test_matmul(self):
        ...
        for lshape, rshape in shapes:
            ...
            # regular matmul
            out = tc.matmul(eteq.link(var), eteq.link(var2))
            ...
            if is_symmetric:
                ...
                both = tc.matmul(eteq.link(var), eteq.link(var))
    ...
    def test_convolution(self):
        ...
        for shape, kernelshape in shapes:
            ...
            out = tc.convolution(eteq.link(var), eteq.link(vkernel), list(range(8)))
            ...
            zero = eteq.derive(out, eteq.link(var2))
            ex = eteq.derive(out, eteq.link(var))
            ex2 = eteq.derive(out, eteq.link(vkernel))
    ...
    def test_conv2d(self):
        ...
        out = tc.nn.conv2d(eteq.link(image), eteq.link(kernel))
        ...
        var2 = eteq.variable(data, 'var2')
        zero = eteq.derive(out, eteq.link(var2))
        ex = eteq.derive(out, eteq.link(image))
        ex2 = eteq.derive(out, eteq.link(kernel))
    ...
    def test_avgpool(self):
        ...
        image = eteq.variable(data, 'image')
        out = tc.nn.mean_pool2d(eteq.link(image), [1, 2])
        sess = eteq.Session()
        sess.track([out])
        sess.update()
        output = out.get()
    ...
    def test_maxpool(self):
        ...
        image = eteq.variable(data, 'image')
        out = tc.nn.max_pool2d(eteq.link(image), [1, 2])
        sess = eteq.Session()
        sess.track([out])
        sess.update()
        output = out.get()
    ...
    def test_grader_scenario1(self): # REDUCE -> MUL
        ...
        out = tc.mul(tc.reduce_sum(eteq.link(var), offset=1, ndims=1), eteq.link(var2))
    ...
    def test_grader_scenario2(self): # EXTEND -> MUL
        ...
        out = tc.mul(tc.extend(eteq.link(var), 1, [3]), eteq.link(var2))
    ...
    def test_grader_scenario3(self): # PERMUTE -> MUL
        ...
        out = tc.mul(tc.permute(eteq.link(var), [1,0]), eteq.link(var2))
    ...
    def test_grader_scenario4(self): # MATMUL -> MUL
        ...
        out = tc.mul(tc.matmul(eteq.link(var), eteq.link(var2)), eteq.link(var3))
    ...
    def test_grader_scenario5(self): # MATMUL -> MATMUL
        ...
        out = tc.matmul(tc.matmul(eteq.link(var), eteq.link(var2)), eteq.link(var3))
    ...
    def test_grader_scenario6(self): # REDUCE -> MATMUL
        ...
        out = tc.matmul(tc.reduce_sum(eteq.link(var), offset=2, ndims=1), eteq.link(var2))
    ...
    def test_grader_scenario7(self): # EXTEND -> MATMUL
        ...
        out = tc.matmul(tc.extend(eteq.link(var), 1, [10]), eteq.link(var2))
    ...
    def test_grader_scenario8(self):
        ...
        out = tc.matmul(
            tc.extend(eteq.link(var), 1, [10]),
            tc.permute(tc.extend(eteq.link(var), 1, [3]), [1, 0]))
    ...
    def test_grader_scenario9(self):
        ...
        d = tc.matmul(eteq.link(a), eteq.link(b))
        e = tc.matmul(eteq.link(c), d)
        f = tc.matmul(tc.transpose(d), tc.transpose(eteq.link(c)))
        dest = tc.matmul(e, f)
        ...
        da = eteq.derive(dest, eteq.link(a))
        db = eteq.derive(dest, eteq.link(b))
        dc = eteq.derive(dest, eteq.link(c))
