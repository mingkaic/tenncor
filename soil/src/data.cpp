...
DataSource::DataSource (DTYPE type, NElemT n) :
	data_(std::shared_ptr<char>(
		(char*) malloc(n * type_size(type)),
		varr_deleter())),
	type_(type) {}

DataSource::DataSource (char* data, DTYPE type, NElemT n) :
	type_(type)
{
	size_t nbytes = n * type_size(type);
	data_ = std::shared_ptr<char>(
		(char*) malloc(nbytes), varr_deleter());
	std::memcpy(data_.get(), data, nbytes);
}

char* DataSource::data (void) const
{
	return data_.get();
}

DTYPE DataSource::type (void) const
{
	return type_;
}
