...
{
	std::stringstream ss;
	err::to_stream(ss, "abcdefghijkl");
	EXPECT_STREQ("abcdefghijkl", ss.str().c_str());
	ss.str("");

	err::to_stream(ss, "abcd\\efgh\\ijkl\\");
...
TEST(STRING, Iterators)
{
	...
	std::string dbstr = err::to_string(dbs.begin(), dbs.end());
	std::string iarstr = err::to_string(iar.begin(), iar.end());
...
TEST(STRING, Sprintf)
{
	...
	std::string s = err::sprintf("%% %s %d %.3f %% %%%% %s %d %.1f %% %%%%",
